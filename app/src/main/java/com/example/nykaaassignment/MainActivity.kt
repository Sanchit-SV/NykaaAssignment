...
fun NewScreen(
    ...
    var retryClicked  by remember {
      mutableStateOf(false)
    }

    var initialApiCalled by rememberSaveable { mutableStateOf(false) }

    if(!initialApiCalled || retryClicked) {
        LaunchedEffect(key1 = retryClicked, block = {
    ...
    when (state.value) {
        ...
        is CatResponseState.Error -> {
            ErrorMessegeWithRetry((state.value as CatResponseState.Error).errorDetails){
                retryClicked = true
            }
        }
